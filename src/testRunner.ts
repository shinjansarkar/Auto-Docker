...
export class DockerTestRunner {
    ...
    /**
     * Run comprehensive tests for all supported technologies
     */
    ...
    private async testMessageQueues(workspaceRoot: string): Promise<void> {
        ...
        for (const queue of queues) {
            await this.testMessageQueue(queue.name, queue.image, queue.port, queue.managementPort);
        }
    }
    ...
    private async testTechnology(
        ...
        try {
            ...
            // Generate Docker files using the extension
            // This would call the actual extension logic
            ...
            this.testResults.push({
                ...
                category,
                status,
                message: status === 'passed' ? 'All checks passed' : 'Some checks failed',
                issues,
                duration,
                containerLogs: logs,
                healthCheck,
    ...
    private async testDatabase(name: string, image: string, port: number): Promise<void> {
        ...
        try {
            ...
            this.testResults.push({
                ...
                category: 'Database',
                status,
                message: status === 'passed' ? 'Database is healthy' : 'Database health check failed',
                issues,
                duration,
                healthCheck,
            });
     ...
     */
    private async testMessageQueue(
        name: string,
        image: string,
        port: number,
        ...
        try {
            ...
            const portMapping = managementPort
                ? `-p ${port}:${port} -p ${managementPort}:${managementPort}`
                : `-p ${port}:${port}`;
            ...
            this.testResults.push({
                ...
                status,
                message: status === 'passed' ? 'Message queue is healthy' : 'Health check failed',
                issues,
                duration,
                healthCheck,
            });
    ...
    private async testSearchEngine(name: string, image: string, port: number): Promise<void> {
        ...
        try {
            ...
            await this.waitForContainer(containerName, 60000); // Search engines take longer
            ...
            this.testResults.push({
                ...
                status,
                message: status === 'passed' ? 'Search engine is healthy' : 'Health check failed',
                issues,
                duration,
                healthCheck,
            });
    ...
    private async testReverseProxy(name: string, image: string, port: number): Promise<void> {
        ...
        try {
            ...
            this.testResults.push({
                ...
                status,
                message: status === 'passed' ? 'Reverse proxy is healthy' : 'Health check failed',
                issues,
                duration,
                healthCheck,
            });
    ...
    private async performHealthCheck(template: string, port?: number): Promise<HealthCheckResult> {
        if (!port) {
            return { isHealthy: false, error: 'No port specified' };
        }
    ...
    private async checkDatabaseHealth(name: string, port: number): Promise<HealthCheckResult> {
        // Implement specific health checks for each database
        return { isHealthy: true };
    }

    private async checkMessageQueueHealth(name: string, port: number, managementPort?: number): Promise<HealthCheckResult> {
        // Implement specific health checks for each message queue
        return { isHealthy: true };
    }

    private async checkSearchEngineHealth(name: string, port: number): Promise<HealthCheckResult> {
        try {
            const response = await fetch(`http://localhost:${port}`);
            return {
                isHealthy: response.ok,
    ...
    private async checkReverseProxyHealth(name: string, port: number): Promise<HealthCheckResult> {
        try {
            const response = await fetch(`http://localhost:${port}`);
            return {
                isHealthy: response.ok || response.status === 404, // 404 is OK for empty proxy
                endpoint: `http://localhost:${port}`,
                statusCode: response.status,
            };
        } catch (error) {
